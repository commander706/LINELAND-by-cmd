            ...
            for (let c = 0; c < cols; c++) {
                ...
                if (tile.type === 'goal') {
                        ...
                        if (colors.length === 1) {
                            ...
                            if (isBlock) {
                                for (let i = 0; i < 8; i++) {
                                    ...
                                    const pX = px + Math.cos(ang) * radius;
                                    const pY = py + Math.sin(ang) * radius;
                                    if (i === 0) this.ctx.moveTo(pX, pY);
                                    else this.ctx.lineTo(pX, pY);
                                }
                            } else {
                                this.ctx.moveTo(px, py - radius); this.ctx.lineTo(px + radius, py); this.ctx.lineTo(px, py + radius); this.ctx.lineTo(px - radius, py);
                            }
                            ...
                            for (let i = 0; i < colors.length; i++) {
                                ...
                                this.ctx.arc(px, py, radius, startAng, endAng);
                                this.ctx.closePath(); this.ctx.fill();
                            }
                            this.ctx.restore();
                        }
                    ...
                    if (tile.type === 'color_block') {
                        for (let i = 0; i < 8; i++) {
                            const ang = i * Math.PI / 4 + Math.PI / 8;
                            const pX = px + Math.cos(ang) * 40 * scale;
                            const pY = py + Math.sin(ang) * 40 * scale;
                            if (i === 0) this.ctx.moveTo(pX, pY);
                            else this.ctx.lineTo(pX, pY);
                        }
                    } else {
                        this.ctx.moveTo(px, py - 40 * scale); this.ctx.lineTo(px + 40 * scale, py); this.ctx.lineTo(px, py + 40 * scale); this.ctx.lineTo(px - 40 * scale, py);
                    }
                    this.ctx.closePath(); this.ctx.fill();

                    this.ctx.strokeStyle = '#ffffff'; this.ctx.lineWidth = 8 * scale; this.ctx.stroke();
                    drawMultiColorShape(tile.type === 'color_block' ? 24 * scale : 28 * scale, tile.type === 'color_block');
