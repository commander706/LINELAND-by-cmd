...
class GameApp {
    ...
    async init() {
        ...
        document.body.addEventListener('click', (e) => {
            ...
            if (t.classList.contains('back-btn') || t.classList.contains('back') || 
                t.id.includes('cancel') || t.id.includes('nosave') || t.id === 'confirm-no-btn') {
            ...
            if (e.key === 'Escape') {
                ...
                if (activeModals.length > 0) {
                    activeModals.forEach(m => {
                        const closeBtn = m.querySelector('.modal-btn:not(.confirm):not(.danger)') || m.querySelector('#setting-close-btn') || m.querySelector('#exit-nosave-btn') || m.querySelector('#modal-cancel-btn');
    ...
    setupEditorEvents() {
        ...
        document.getElementById('btn-play')?.addEventListener('click', async () => {
            ...
            if (this.playEngine.loadLevel(lvl.data, {title: lvl.title, subtitle: lvl.subtitle, author: lvl.author}, false)) {
                const hideLoading = await this.ui.showFakeLoading(800);
                document.getElementById('editor-select-screen').classList.remove('active');
                document.getElementById('editor-select-screen').style.opacity = '0';
        ...
        document.getElementById('edit-test-btn')?.addEventListener('click', async () => {
            ...
            if (this.playEngine.loadLevel(levelData, {title, subtitle, author}, true)) {
                const hideLoading = await this.ui.showFakeLoading(600);
